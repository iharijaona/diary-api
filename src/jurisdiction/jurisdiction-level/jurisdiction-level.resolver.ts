...
export class JurisdictionLevelResolver {
  constructor(private readonly jurisdictionLevelService: JurisdictionLevelService) {}
  
  @Query(() => JurisdictionLevel)
  async jurisdictionLevel(@Args('code', { type: () => EnumJurisdictionLevel }) code: string): Promise<JurisdictionLevel> {
    const jurisdictionLevel = await this.jurisdictionLevelService.findOneJurisdictionLevel(code);
